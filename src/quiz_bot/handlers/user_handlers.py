...
def _profile_line(value: object) -> str:
...
async def handle_my_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.exception("SQLite error while opening profile user_id=%s", update.effective_user.id)
        await update.message.reply_text(translate("en", "db_error"))
        return ConversationHandler.END
    ...
    try:
        profile = await adb_run(lambda conn: load_profile_db(conn, update.effective_user.id))
    except sqlite3.Error:
        logger.exception("SQLite error while loading profile user_id=%s", update.effective_user.id)
        await update.message.reply_text(translate(language_code, "db_error"))
        return ConversationHandler.END

    if profile is None:
        text = translate(language_code, "my_info_empty")
    else:
        text = "\n".join(
            (
                translate(language_code, "my_info_title"),
                translate(language_code, "my_info_row", label=translate(language_code, "profile_name_label_short"), value=_profile_line(profile["profile_full_name"])),
                translate(language_code, "my_info_row", label=translate(language_code, "profile_phone_label_short"), value=_profile_line(profile["profile_phone_number"])),
                translate(language_code, "my_info_row", label=translate(language_code, "profile_study_label_short"), value=_profile_line(profile["profile_study_or_address"])),
            )
        )
    await update.message.reply_text(text, reply_markup=main_reply_keyboard(language_code, is_admin=await _is_admin(update.effective_user.id)))
    return ConversationHandler.END
